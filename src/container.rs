//! MP3 file container.
//!
//! MP3 has no real container — files are concatenations of MPEG audio
//! frames, typically with an ID3v2 tag prefix and an optional ID3v1
//! suffix. The "container" here:
//!
...
//! 3. Emits each frame as one `Packet` (the MP3 decoder consumes one
//!    frame per packet).
//!
...
fn probe(p: &ProbeData) -> u8 {
    ...
    for i in 0..scan_end.saturating_sub(4) {
        if p.buf[i] == 0xFF
            && (p.buf[i + 1] & 0xE0) == 0xE0
            && parse_frame_header(&p.buf[i..i + 4]).is_ok()
        {
...
fn find_first_frame(input: &mut Box<dyn ReadSeek>) -> Result<([u8; 4], usize)> {
    ...
    while i + 4 <= buf.len() {
        if buf[i] == 0xFF && (buf[i + 1] & 0xE0) == 0xE0 {
            ...
            if parse_frame_header(&head).is_ok() {
                return Ok((head, i));
            }
        }
        i += 1;
...
impl Demuxer for Mp3Demuxer {
    ...
    fn next_packet(&mut self) -> Result<Packet> {
        ...
        let hdr = match parse_frame_header(&head) {
            Ok(h) => h,
