//! Integration tests for the round-305 §2.4.3.4.9.2 + §2.4.3.4.9.3
//! **MS + short-block + intensity-stereo** encoder path (ISO/IEC
//! 11172-3:1993 + ISO/IEC 13818-3:1997 §2.4.3.2 per-window bound).
//!
//! r303 wired the per-window short-block intensity bound on the
//! intensity-only force-short path
//! ([`oxideav_mp3::Mp3Encoder::new_joint_stereo_is`] +
//! [`oxideav_mp3::Mp3Encoder::force_short_blocks_for_testing`]). r305
//! lifts the `IntensityShortBlocksUnsupported` rejection on the
//! **unconditional MS + intensity** force-short path
//! ([`oxideav_mp3::Mp3Encoder::new_joint_stereo_ms_is`]): the
//! §2.4.3.4.9.2 MS rotation is now applied per window below each
...
//!    (joint stereo), `mode_extension = '11'` (MS + intensity), and
//!    every granule-channel is a pure short block.
//! 2. **Wire-level positions** — the right channel's
//!    `scalefac_s[sfb][win]` carries legal stereo positions (`is_pos ∈
//!    0..=6`) or the illegal marker `7`; nothing else.
//! 3. **MS fidelity below the bound** — a below-bound 440 Hz tone whose
//!    L/R pan is encoded through the MS matrix reconstructs at its true
//!    pan after a spec-order self-decode (huffman → requantize →
//!    reorder → process_stereo → alias → imdct → synth), and a hard-left
//!    intensity-region tone reconstructs left-leaning.
...
/// windows. The intensity right channel carries `scalefac_compress = 15`
...
/// Stereo PCM: a below-bound 440 Hz tone with a mild L>R pan (carried
/// through the MS matrix) plus a high tone for the intensity region.
...
#[test]
fn ms_short_intensity_header_and_short_side_info() {
    let pcm = two_tone_stereo_pcm(SR as usize / 2, false);
    let out = encode_ms_short_is(&pcm);

    let mut frames = 0usize;
    for frame in FrameWalker::new(&out) {
        let hdr = parse_header(&frame.data[..4]).expect("header");
        assert!(matches!(hdr.mode, ChannelMode::JointStereo));
        assert_eq!(hdr.mode_extension.raw, 0b11, "MS + intensity header");
...
#[test]
fn ms_short_intensity_right_channel_positions_in_range() {
...
#[test]
fn ms_short_intensity_self_decode_preserves_image() {
    let n = SR as usize;
    ...
    // Intensity-region (hard-left high tone) reconstructs left-leaning.
    ...
    eprintln!("ms+short+intensity 8 kHz reconstructed |L|/|R| = {hi_ratio:.3}");
    assert!(
        hi_ratio > 2.0,
        "coupled hard-left high tone should reconstruct left-leaning, got {hi_ratio}"
    );

    // Below-bound 440 Hz tone, carried through the MS matrix, keeps its
    // true ~1.4 pan.
    let low_ratio = (goertzel_power(seg_l, 440.0) / goertzel_power(seg_r, 440.0).max(1.0)).sqrt();
    eprintln!("ms+short+intensity 440 Hz reconstructed |L|/|R| = {low_ratio:.3}");
    assert!(
        (1.05..=2.0).contains(&low_ratio),
        "below-bound MS pan drifted: {low_ratio}"
    );
}

#[test]
fn ms_short_intensity_encode_is_bit_exact() {
    let pcm = two_tone_stereo_pcm(SR as usize / 4, false);
    let a = encode_ms_short_is(&pcm);
    let b = encode_ms_short_is(&pcm);
